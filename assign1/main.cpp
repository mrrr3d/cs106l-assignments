...
void write_courses_offered(std::vector<Course> &all_courses) {
  ...
  std::vector<Course> to_del;
  for (auto &t : all_courses) {
    if (t.quarter != "null") {
      ofs << t.title << ",";
      ofs << t.number_of_units << ",";
      ofs << t.quarter;
      ofs << "\n";
      to_del.push_back(t);
    }
  ...
  for (auto &t : to_del) {
    delete_elem_from_vector(all_courses, t);
  }
}
